using Assets.Samples.Fog.Runtime;
using UnityEditor;
using UnityEditor.Rendering;

namespace Assets.Samples.Fog.Editor
{
    ...
    sealed class FullScreenFogEditor : VolumeComponentEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (FullScreenFog.UseStartLine(mode)) PropertyField(_startLine);
            if (FullScreenFog.UseEndLine(mode, densityMode)) PropertyField(_endLine);
            if (FullScreenFog.UseStartHeight(mode)) PropertyField(_startHeight);
            if (FullScreenFog.UseEndHeight(mode, densityMode)) PropertyField(_endHeight);
            if (FullScreenFog.UseIntensity(densityMode)) PropertyField(_density);

            var noiseMode = (FullScreenFogNoiseMode)_noiseMode.value.intValue;
            PropertyField(_noiseMode);

            if (FullScreenFog.UseNoiseTex(noiseMode)) PropertyField(_noiseTexture);
